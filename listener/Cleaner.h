 ...
 *
 * @class Cleaner
 * @brief This is only place, where signals are handled.
 * 	      Cleaner waits for some signals and cleans after
 * 	      listeners (child processes).
 * 	      Start should be called in seperated thread.
 * @date 29-05-2012
 */
...
private:
	...
	std::map<pid_t, pthread_t> procs_;
	/** mutex to lock modifcations on procs_ */
	pthread_mutex_t procs_mutex_;
